// Implementation that works with Socket.IO lib on Unity side
...
io.on('connection', (socket) => {
    ...
    socket.on('start-game-gm', () => {
        console.log("Emitting start-game event to all clients")
        io.emit('start-game')
    });

    socket.on('disconnect', () => {
        console.log('A user disconnected' + JSON.stringify(connectedUsers[socket.id]));
    });
});
